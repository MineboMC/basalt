...
)
{
    ...
    override fun getGrantsLore(player: Player, rankGrant: RankGrant): MutableList<String>
    {
        ...
        val lore = BasaltSpigotPlugin.instance.config.getStringList(key)
        if (scope.global)
        {
            servers.add("global")
        } else
        ...
        for (i in 0 until lore.size)
        {
            lore[i] = Chat.format(lore[i]
                ...
                .replace("<target>", BasaltAPI.getRankDisplay(rankGrant.target))
                ...
                .replace("<scopes>", servers
                    ...
                    .joinToString("\n"))
                .replace("<issued_by>", BasaltAPI.getRankDisplay(rankGrant.executor))
                .replace("<issued_reason>", rankGrant.addedReason)
                .replace("<duration>", TimeUtil.formatDuration(rankGrant.duration))
                .replace(
                    "<removed_by>",
                    if (rankGrant.removedBy == null) "Unknown" else BasaltAPI.getRankDisplay(rankGrant.removedBy!!)
                )
                .replace(
                    "<removal_reason>",
    ...
    override fun getGrantsData(player: Player, rankGrant: RankGrant): Short
    {
        ...
        return BasaltSpigotPlugin.instance.config.getInt(key).toShort()
    }
