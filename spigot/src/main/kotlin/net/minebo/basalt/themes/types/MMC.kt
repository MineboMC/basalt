...
)
{
    ...
    override fun getGrantsLore(player: Player, rankGrant: RankGrant): MutableList<String>
    {
        ...
        desc.add(Chat.format("&eTarget: &r" + BasaltAPI.getRankDisplay(rankGrant.target)))
        ...
        desc.add(Chat.format("&6&m-------------------------------------"))
        desc.add(Chat.format("&eIssued By: &f" + BasaltAPI.getRankDisplay(rankGrant.executor)))
        ...
        if (!rankGrant.expirable.isActive())
        {
            ...
            {
                desc.add(Chat.format("&eRemoved By: &f" + BasaltAPI.getRankDisplay(rankGrant.removedBy!!)))
            }
        ...
        if (!player.hasPermission("basalt.grants.remove") && rankGrant.getGrantable().weight >= ((BasaltAPI.syncFindProfile(
                player.uniqueId
            )?.getCurrentRank()?.weight) ?: 0)
        )
        {
            desc.add(Chat.format("&cYou don't have permission to remove this grant"))
        } else if (player.hasPermission("basalt.grants.remove"))
        {
            desc.add(Chat.format("&aRight-Click to remove this grant from &r" + BasaltAPI.getRankDisplay(rankGrant.target)))
        }

        if (player.hasPermission("basalt.grants.scopes.audit"))
        {
    ...
    override fun getHistoryLore(player: Player, punishment: Punishment): MutableList<String>
    {
        ...
        desc.add(Chat.format("&6&m-------------------------------------"))
        desc.add(Chat.format("&eTarget: &r" + BasaltAPI.getRankDisplay(punishment.target)))
        ...
        desc.add(Chat.format("&6&m-------------------------------------"))
        desc.add(Chat.format("&eIssued By: &f" + BasaltAPI.getRankDisplay(punishment.executor)))
        desc.add(Chat.format("&eIssued Reason: &f" + punishment.reason))
        desc.add(Chat.format("&6&m-------------------------------------"))
        if (!punishment.expirable.isActive())
        {
            desc.add(Chat.format("&eRemoved By: &f" + BasaltAPI.getRankDisplay(punishment.removedBy!!)))
    ...
    {
        return BasaltAPI.getWoolColor(punishment.color).woolData.toShort()
    }
}