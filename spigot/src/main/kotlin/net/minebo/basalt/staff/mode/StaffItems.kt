...
object StaffItems
{
    ...
    val WORLDEDIT_AXE = ItemBuilder.of(Material.WOODEN_AXE).name("&bWorldedit Axe").build()
    val BETTER_VIEW = ItemBuilder.of(Material.GRAY_CARPET).name("&bBetter view").build()
    val VANISH = ItemBuilder.of(Material.LIME_WOOL).name("&bUnvanish").build()
    val UNVANISH = ItemBuilder.of(Material.GRAY_WOOL).name("&bVanish").build()
    ...
    fun equip(player: Player)
    {
        ...
        resource.use {
            val item = it.hget("Basalt:ModMode:", player.uniqueId.toString())
            ...
            } else
            {
                ...
                if (player.hasPermission("basalt.staffmode.worldedit"))
                {
